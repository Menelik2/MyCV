...
function initTheme() {
  ...
  // Default to light mode on first visit; only force dark if user chose it
...
function openWindow(id) {
  ...
  // Compact floating size — never open maximized / full-screen
  const APP_SIZES = {
    notepad: { w: 480, h: 340 },
    paint: { w: 520, h: 380 },
  ...
  const def = isApp
    ...
    : { w: 440, h: 360 };
  const maxW = Math.min(def.w, window.innerWidth - 80);
  const maxH = Math.min(def.h, window.innerHeight - 80);
...
function tryLoadProfile() {
  ...
  const primary = supportsWebP ? "profile.webp" : "profile.jpg";
  ...
  img.onerror = () => {
    if (primary !== fallback) {
      ...
      img2.onerror = () => console.warn("profile image not found — using initials fallback");
      img2.src = fallback;
    } else {
...
function requestOsFullscreen() {
  ...
  document.documentElement.classList.add("os-fullscreen");
}
...
async function finishBoot() {
  ...
  setTimeout(() => {
    boot?.remove();
    if (window.innerWidth >= 900) openWindow("about");
  }, 650);
}
...
function runBootSequence() {
  ...
  const steps = [
    { p: 15, msg: "Loading personal settings…" },
    { p: 35, msg: "Preparing desktop…" },
    { p: 55, msg: "Starting services…" },
    { p: 78, msg: "Almost ready…" },
    { p: 100, msg: "Welcome" }
  ];
  let i = 0;
  const tick = () => {
  ...
  document.addEventListener("keydown", function onKey(e) {
    if (startBtn && !startBtn.hidden && (e.key === "Enter" || e.key === " ")) {
      e.preventDefault();
...
const ADMIN_SALT = "menelik-xp-portfolio-v1";
// Factory default is stored only as a hash (password never appears in source)
const DEFAULT_ADMIN_HASH = "27c27ad51be55a69c57b8cf995e3098299e437af9a135db6a04a5f5ae4c29ce8";
...
async function hashPassword(password) {
  const data = new TextEncoder().encode(ADMIN_SALT + "\0" + password);
...
async function verifyAdminPassword(password) {
  const expected = await getStoredPassHash();
...
function initAdminUI() {
  ...
  document.getElementById("admin-save")?.addEventListener("click", () => {
    ...
    // Invalidate mobile cache for this section
    ...
    st.textContent = "Saved. Content updated in this browser.";
  });
  ...
  document.getElementById("admin-export")?.addEventListener("click", () => {
    ...
    document.getElementById("admin-status").textContent = "Exported portfolio-content.json";
  });
  ...
  document.getElementById("admin-change-pass")?.addEventListener("click", async () => {
    ...
    if (weak) {
      st.className = "admin-status err";
      st.textContent = "Password is too weak. Use a mix of characters.";
      return;
    }
    ...
    st.className = "admin-status";
    st.textContent = "Password updated (hashed). Use it next time you log in on this browser.";
  });
